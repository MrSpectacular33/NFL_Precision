...
if __name__ == "__main__":
    ...
    ap.add_argument("--prob-col", type=str, default="p_cal", help="column to use for probability (default p_cal)")
    ap.add_argument("--price-col", type=str, default="price", help="column to use for price (default price)")
    args = ap.parse_args()

    df = pd.read_csv(args.infile)

    # select prob + price columns safely
    if args.prob_col not in df.columns:
        raise SystemExit(f"Missing prob column: {args.prob_col}")
    df = df.copy()
    df["p_cal"] = df[args.prob_col].clip(0.01, 0.99)
    if args.price_col in df.columns:
        df["price"] = df[args.price_col]
