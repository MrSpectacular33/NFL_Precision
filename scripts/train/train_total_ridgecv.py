...
def main():
    ...
    if "market_total" in X:
        ...
        season_mean = mt.groupby(df["season"]).transform("mean")
    ...
    with open("reports/cv/total_ridgecv.json","w",encoding="utf-8") as f:
        json.dump({"target":"total_points","n":int(len(y)),"xcols":XCOLS,"alpha":float(a),"rmse_cv":float(rmse_cv),"rmse_in":rmse_in}, f, indent=2)
    print(f"[train_total_ridgecv] alpha={a:.4g} RMSE_CV={rmse_cv:.3f} n={len(y)} -> reports/cv/total_ridgecv.json")
