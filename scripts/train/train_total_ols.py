...
def ols(df: pd.DataFrame, target: str, xcols: list[str], out_path: str):
    ...
    if "market_total" in X:
        mt = _as_num_series(df.get("market_total", pd.Series(index=df.index, dtype="float64")), df.index)
        ...
        if X["market_total"].isna().all():
            X = X.drop(columns=["market_total"])
    ...
    if all_nan_cols:
        print(f"[train_total_ols] dropping all-NaN cols: {all_nan_cols}")
    ...
    print(f"[train_total_ols] NA cells before fill: {na_before}, after fill: {X.isna().sum().sum()}")

    y = _as_num_series(df.get(target), df.index)
    if y.notna().sum() == 0:
        print("[train_total_ols] target missing in lag file; reconstructing from base…")
...
if __name__ == "__main__":
    ...
    rmse, n = ols(df, "total_points", XCOLS, "reports/cv/total_ols.json")
    print(f"[train_total_ols] RMSE={rmse:.3f} n={n} -> reports/cv/total_ols.json")
