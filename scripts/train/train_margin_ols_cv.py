...
def debug(msg):
    print(f"[margin_ols_cv] {msg}", flush=True)
...
def build_X(df, xcols):
    ...
    if "market_spread" in X:
        ms = pd.to_numeric(df.get("market_spread", pd.Series(index=df.index)), errors="coerce")
...
def main():
    ...
    X = build_X(df, XCOLS)
    y = pd.to_numeric(df.get("margin"), errors="coerce")
    ...
    out = {"target":"margin","n":int(len(y)),"xcols":XCOLS,"rmse_cv":rmse_cv}
    pathlib.Path("reports/cv").mkdir(parents=True, exist_ok=True)
    outp = "reports/cv/margin_ols_cv.json"
    with open(outp, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)
    print(f"[train_margin_ols_cv] RMSE_CV={rmse_cv:.3f} n={len(y)} -> {outp}")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"[margin_ols_cv][ERROR] {type(e).__name__}: {e}", file=sys.stderr)
        raise
