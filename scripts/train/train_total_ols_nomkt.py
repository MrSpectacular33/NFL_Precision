...
OUT = pathlib.Path("reports/cv/total_ols_nomkt.json")
...
XCOLS = [
    ...
    # EXCLUDE market_total
]

def main():
    df = pd.read_parquet(SRC)

    y = pd.to_numeric(df.get("total_points", pd.Series(index=df.index, dtype="float64")), errors="coerce")
    ...
    print(f"[train_total_ols_nomkt] RMSE={rmse:.3f} n={out['n']} -> {OUT}")
